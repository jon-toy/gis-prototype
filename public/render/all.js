...
function initZones() {
  ...
  var starting_pos = new google.maps.LatLng(35.33, -109.45); // Starting position
  ...
  map.data.addListener("click", function (event) {
    ...
    $("#navbar-title").html(
      event.feature.getProperty("ZONE_NAME") + " Parcel Viewer"
    );
    initParcels(zone);
  });
...
function initParcels(zone_num, starting_lat_lon, callback) {
  ...
  $.get(uri, function (data, status) {
    ...
    // Create the Map object
    if (
      starting_lat_lon == null &&
      data.body.starting_lat &&
      data.body.starting_lon
    )
      ...
      $.getJSON(geo_json_urls[i], function (data) {
        ...
        if (load_completed.length == geo_json_urls.length) {
          ...
          document.getElementById("loading-message-status").innerHTML =
            "Loading Parcel Data (" +
            load_completed.length +
            " of " +
            geo_json_urls.length +
            ")...";
        }
      });
    }
...
function showFeature(feature) {
  ...
  document.getElementById("button-link-assessor").href =
    "https://eagleassessor.co.apache.az.us/assessor/taxweb/account.jsp?guest=true&accountNum=" +
    account_number;
  document.getElementById("button-link-treasurer").href =
    "https://eagletreasurer.co.apache.az.us:8443/treasurer/treasurerweb/account.jsp?guest=true&account=" +
    account_number;
...
function getZoneName(zone_num) {
  ...
  if (all_zones.length == 0) {
    $.getJSON("https://apachecounty.org/zones/zones.json", function (data) {
      ...
      for (var i = 0; i < all_zones.length; i++) {
        if (all_zones[i].getProperty("ZONE") == zone_num) {
          $("#navbar-title").html(
            all_zones[i].getProperty("ZONE_NAME") + " Parcel Viewer"
          );
          $("#confirmZoneModalNewZone").html(
            all_zones[i].getProperty("ZONE_NAME")
          );
    ...
    for (var i = 0; i < all_zones.length; i++) {
      if (all_zones[i].getProperty("ZONE").toString() == zone_num.toString())
        return all_zones[i].getProperty("ZONE_NAME");
    }
  }
